...
> 회원 탈퇴 같은 경우 `User.findByIdAndDelete`를 사용하여 데이터를 삭제할 수 있다. 물론 정보보안 측면에서는 탈퇴한 사용자의 모든 정보가 DB에서 삭제되어야 한다고 볼 수도 있지만, 탈퇴한 회원의 정보 역시 바로 테이블에서 삭제하는 것이 아니라 설계에 따라 각 회원들이 설정한 회원 정보 보관 정책 기간 동안 비활성화가 필요하거나 일정 기간 관리자의 실수로 인해 데이터 백업이 필요한 경우 등에 따라 일부 데이터 백업 로직이 필요할 수 있다.
